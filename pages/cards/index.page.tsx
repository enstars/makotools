...
function Page({
  ...
  cards: LocalizedLoadedDataArray<GameCard, GameCardStrings>;
  characters: LocalizedLoadedDataArray<GameCharacter, GameCharacterStrings>;
}) {
...
export const getServerSideProps = getServerSideUser(async ({ res, locale }) => {
  const characters = await getLocalizedData<
    GameCharacter,
    GameCharacterStrings
  >("characters", locale, "character_id", [
    "character_id",
    "first_name",
    "sort_id",
  ]);
  ...
  const cards = await getLocalizedData<GameCard, GameCardStrings>(
    "cards",
    locale,
    "id",
    [
